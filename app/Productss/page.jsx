...
const productData = {
  "Chute Type": {
    ...
    "4 Chute 256 Channels": {
      images: [
        "https://ik.imagekit.io/bxhlcmkcf/Urent/Screenshot%202025-08-26%20170426.png?updatedAt=1756208197557",
      ],
      link: "/ColorSorters/Chute-type",
    },
    ...
    "7 Chute 448 Channels": {
      images: [
        "https://ik.imagekit.io/bxhlcmkcf/Urent/img72.png?updatedAt=1756188638095",
      ],
      link: "/ColorSorters/Chute-type",
    },
...
const Page = () => {
  ...
  const filteredProducts = (() => {
    // Case 1: Both group + type selected → show only that product
    if (selected && activeGroup && productData[activeGroup][selected]) {
      return [
        {
          group: activeGroup,
          label: selected,
    ...
    // Case 2: Only group selected → show all products in that group
    if (activeGroup && productData[activeGroup]) {
      return Object.entries(productData[activeGroup]).map(([label, data]) => ({
        group: activeGroup,
        label,
        ...data,
      }));
    ...
    // Case 3: Nothing selected → show everything
    return allProducts;
  })();
              ...
              {Object.entries(productData).map(([group, types], i) => (
                <div key={i}>
                  ...
                  <h3
                    ...
                    className={`cursor-pointer text-white bg-green-700 font-semibold px-4 py-2 ${
                      activeGroup === group && !selected ? "bg-green-900" : ""
                    }`}
                  >
                    {group}
                  </h3>
                  <ul>
                    {Object.keys(types).map((type, j) => (
                      <li
                        key={j}
                        onClick={() => {
                          if (activeGroup === group && selected === type) {
                            setActiveGroup(null);
                            setSelected(null);
                          } else {
                            setActiveGroup(group);
              ...
              <Link key={i} href={product.link}>
                <div className="rounded-lg bg-white shadow-md hover:shadow-lg transition p-4 cursor-pointer">
                  ...
                  <p className="text-sm text-gray-500">
                    Provide you with a full set of color sorting quality upgrade solutions
                  </p>
                </div>
              </Link>
            ))}
